...
function calculateWeightedFrequency(freq) {
	...
	const normalized = (weightedScore / maxScore) * 100; // Now it's 0 to 100
...
export const Question = React.memo(({ idx, question }) => {
	const colorMap = [
		"text-green-500",
		"text-green-500",
		"text-yellow-500",
		...
		>
			...
			<div className="flex items-center gap-3">
				...
				<p
					...
					onClick={(e) => {
						addDoneQuestion(question.titleSlug, !doneQuestion);
				...
				>
					<CheckCircle
						className={`${
							doneQuestion ? "text-green-400" : "text-gray-500"
						}`}
					/>
				</p>
